...
def msg_nocmd(update: Update, context: CallbackContext):
    ...
    #if update_msg.from_user.username is not None:
    #    user_name = "@{}".format(update_msg.from_user.username)
    if update_msg.from_user.id is not None:
        user_name = "[{}](tg://user?id={})".format(escape_markdown(user_name), update_msg.from_user.id)
    ...
    if solve_num.lower() in msg_text.lower():
        ...
        welcome_msg = get_chat_config(chat_id, "Welcome_Msg").format(user_name, escape_markdown(chat_title))
        if welcome_msg != "-":
...
def receive_poll_answer(update: Update, context: CallbackContext):
    ...
    chat_title = poll_data["chat"]["title"]
    ...
    if query.from_user.id is not None:
        user_name = "[{}](tg://user?id={})".format(escape_markdown(user_name), query.from_user.id)
    # Get chat settings
    lang = get_chat_config(chat_id, "Language")
    rm_result_msg = get_chat_config(chat_id, "Rm_Result_Msg")
    rm_welcome_msg = get_chat_config(chat_id, "Rm_Welcome_Msg")
...
def button_request_pass(bot, query):
    ...
    #bot_msg = TEXT[lang]["CAPTCHA_SOLVED"].format(user_name)
    #if rm_result_msg:
    #    tlg_send_selfdestruct_msg_in(bot, chat_id, bot_msg, CONST["T_DEL_WELCOME_MSG"], parse_mode="MARKDOWN")
    #else:
    #    tlg_send_msg(bot, chat_id, bot_msg, "MARKDOWN")
    del new_users[chat_id][user_id]
    # Check for custom welcome message and send it
    welcome_msg = get_chat_config(chat_id, "Welcome_Msg").format(user_name, escape_markdown(chat_title))
    if welcome_msg != "-":
...
def cmd_start(update: Update, context: CallbackContext):
    ...
    if chat_type == "private":
        tlg_send_msg(bot, chat_id, TEXT["VN"]["START"])
    else:
        lang = get_chat_config(chat_id, "Language")
...
def cmd_help(update: Update, context: CallbackContext):
    ...
    chat_type = update_msg.chat.type
    if chat_type == "private":
        tlg_send_msg(bot, chat_id, TEXT["VN"]["HELP"])
    else:
        lang = get_chat_config(chat_id, "Language")
        tlg_msg_to_selfdestruct(update_msg)
...
def cmd_commands(update: Update, context: CallbackContext):
    ...
    chat_type = update_msg.chat.type
    if chat_type == "private":
        tlg_send_msg(bot, chat_id, TEXT["VN"]["COMMANDS"])
    else:
        lang = get_chat_config(chat_id, "Language")
        tlg_msg_to_selfdestruct(update_msg)
...
def cmd_welcome_msg(update: Update, context: CallbackContext):
    ...
    welcome_msg = welcome_msg.replace("$group", "{1}")
...
def cmd_version(update: Update, context: CallbackContext):
    ...
    chat_type = update_msg.chat.type
    if chat_type == "private":
        msg_text = TEXT["VN"]["VERSION"].format(CONST["VERSION"])
        tlg_send_msg(bot, chat_id, msg_text)
    else:
        lang = get_chat_config(chat_id, "Language")
...
def cmd_about(update: Update, context: CallbackContext):
    ...
    chat_type = update_msg.chat.type
    if chat_type == "private":
        msg_text = TEXT["VN"]["ABOUT_MSG"].format(CONST["DEVELOPER"], CONST["REPOSITORY"],
